...
def send_otp_email(email: str, otp: str) -> bool:
    ...
    # Gmail displays app passwords with spaces ("abcd efgh ijkl mnop") but SMTP
    # auth needs the raw 16 chars — strip whitespace so either paste style works
    smtp_password = "".join(os.getenv("SMTP_PASSWORD", "").split())
    smtp_from = os.getenv("SMTP_FROM", "Quantra Security <no-reply@quantra.ai>").strip()
    ...
    try:
        ...
        log.error(
            "SMTP auth REJECTED for user %s: %s — check that SMTP_PASSWORD is an App Password "
            "generated by that exact Google account (myaccount.google.com/apppasswords)",
            smtp_user, e,
        )
        log.warning("[OTP FALLBACK] Please use this OTP code to complete sign up: %s", otp)
        return False
    except Exception as e:
        log.error("Failed to send OTP Email to %s: %s: %s", email, type(e).__name__, e)
        log.warning("[OTP FALLBACK] Please use this OTP code to complete sign up: %s", otp)
        return False
